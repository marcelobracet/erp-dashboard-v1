...
function isRolePermissionsMap(input: unknown): input is RolePermissionsMap {
  ...
  for (const [role, perms] of Object.entries(input as Record<string, unknown>)) {
 ...
 */
export async function fetchPermissionsFromApi(accessToken: string): Promise<RolePermissionsMap | null> {
  ...
  try {
    ...
    // aceita tanto { rolePermissions: {...} } quanto o objeto direto
    const maybeMap =
