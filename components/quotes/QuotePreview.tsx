...
export default function QuotePreview({ quote }: { quote: Quote }) {
  const clientName =
    quote.client_snapshot?.name ?? quote.client?.name ?? "Cliente";
  ...
  const envSections = useMemo(() => {
    ...
    function itemsForEnv(env: {
      id: string;
      name: string;
      item_ids: string[];
    }): QuoteItem[] {
      const byExplicitId = env.item_ids
        .map((id) => byId.get(id))
    ...
    if (quote.environments && quote.environments.length > 0) {
      return quote.environments.map((env) => {
        ...
        const subtotal = envItems.reduce(
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const lsLogo = String(
          window.localStorage.getItem("logo_url") ?? "",
        ).trim();
    ...
    <div className="bg-background text-foreground print:bg-white print:text-[#050a30] rounded-xl border border-glass-10 overflow-hidden">
      ...
      <div className="p-6 border-b border-glass-10 print:border-[#e6eaf7]">
        <div className="flex items-start justify-between gap-6">
          <div>
            <div className="text-xs uppercase tracking-wider text-[#8c756a]">
              Orçamento
            </div>
            <div className="text-2xl font-bold mt-1">
              #{quote.id.slice(0, 8).toUpperCase()}
            </div>
            <div className="text-sm text-text-60 print:text-[#050a30]/70 mt-1">
              Data: {created}
            </div>
          </div>
          ...
          <div className="flex flex-col items-end gap-2">
            ...
            <div className="text-right">
              ...
              <div className="text-xs text-text-60 print:text-[#050a30]/70">
                Proposta de fornecimento e instalação
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="rounded-lg border border-glass-10 print:border-[#e6eaf7] p-4">
            ...
            {quote.client_snapshot?.phone && (
              <div className="text-sm text-text-60 print:text-[#050a30]/70">
                Telefone: {quote.client_snapshot.phone}
              </div>
            )}
            {quote.client_snapshot?.email && (
              <div className="text-sm text-text-60 print:text-[#050a30]/70">
                Email: {quote.client_snapshot.email}
              </div>
            )}
            {quote.client_snapshot?.address && (
              <div className="text-sm text-text-60 print:text-[#050a30]/70">
                Endereço: {quote.client_snapshot.address}
              </div>
            )}
          </div>
          ...
          <div className="rounded-lg border border-glass-10 print:border-[#e6eaf7] p-4">
            ...
            <div className="mt-2 flex items-center justify-between text-sm">
              <span className="text-text-60 print:text-[#050a30]/70">
                Status
              </span>
            ...
            <div className="mt-3 pt-3 border-t border-glass-10 print:border-[#e6eaf7] flex items-center justify-between">
              <span className="text-sm text-text-60 print:text-[#050a30]/70">
                Total
              </span>
      ...
      <div className="p-6">
        <div className="space-y-6">
          {envSections.map((env) => (
            <div
              key={env.id}
              className="rounded-xl border border-glass-10 print:border-[#e6eaf7] overflow-hidden"
            >
                      ...
                      <tr key={it.id}>
                        <td className="py-3 px-4 align-top">
                          <div className="font-medium">
                            {it.product?.name ?? it.description ?? "Item"}
                          </div>
                          {it.description && (
                            <div className="text-xs text-text-60 print:text-[#050a30]/65 mt-1">
                              {it.description}
                            </div>
                          )}
                          {it.note && (
                        ...
                        <td className="py-3 px-4 text-text-60 print:text-[#050a30]/75 align-top">
                          {itemDims(it)}
                        </td>
                        ...
                        <td className="py-3 px-4 text-right font-semibold align-top">
              ...
              <div className="flex items-center justify-end gap-3 px-4 py-3 border-t border-glass-10 print:border-[#e6eaf7] bg-glass-5 print:bg-white">
                <span className="text-xs uppercase tracking-wider text-[#8c756a]">
                  Subtotal
                </span>
        ...
        {quote.notes && (
          <div className="mt-6 rounded-lg border border-glass-10 print:border-[#e6eaf7] p-4">
            ...
            <div className="mt-2 text-sm text-text-80 print:text-[#050a30]/80 whitespace-pre-wrap">
              {quote.notes}
            </div>
          </div>
        )}
        ...
        <div className="mt-8 text-xs text-text-60 print:text-[#050a30]/60">
          Valores sujeitos à confirmação de medidas em obra. Prazo e condições a
          combinar.
        </div>
      </div>
    </div>
