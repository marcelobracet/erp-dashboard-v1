...
function RoleBadge({ role }: { role: string }) {
  ...
  return (
    <span
      className={`px-2 py-0.5 rounded-full text-xs font-medium border ${cls} capitalize`}
    >
      {labels[role] ?? role}
    </span>
  );
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-text-80 mt-0.5">
                Usuários, perfis e acessos ao sistema.
              </p>
            </div>
            {hasPermission('users', 'create') && (
      ...
      >
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            {formErrors.role && <p className="text-sm text-red-400">{formErrors.role}</p>}
          </div>
        </div>
      </Modal>
    </>
